...
diagram runtime diag header footer env [path, title, code] = do
    ...
    let (gv, dat, img, url, attrs) = parseImageAttributes path'
...
-- are added after the link in the mrkdown output.
parseImageAttributes :: String -> (FilePath, FilePath, FilePath, String, String)
parseImageAttributes s = ( localPath++".gv"
                         , localPath++".dat"
                         , localPath++".png"
                         , linkPath++".png"
                         , attrs)
    where
        (localPath, linkPath, attrs) = parseLocalAndLink s "" ""
