...
diagram runtime diag header footer env [path, title, code] = do
    ...
    let (gv, dat, img, url) = splitImgUrl path' True
...
-- splitImgUrl "(x/y/)z" == ("x/y/z.gz", "x/y/z.dat", "x/y/z.png", "z.png")
splitImgUrl :: String -> Bool -> (String, String, String, String)
splitImgUrl (c:cs) both
    | c `elem` "(["     = splitImgUrl cs False
    | c `elem` ")]"     = splitImgUrl cs True
    | both              = (c:gv, c:dat, c:img, c:url)
    | otherwise         = (c:gv, c:dat, c:img, url)
    where (gv, dat, img, url) = splitImgUrl cs both
splitImgUrl [] _ = (".gv", ".dat", ".png", ".png")
