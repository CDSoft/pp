...
data DiagramRuntime = Graphviz | PlantUML | Asymptote | R deriving (Show)
...
builtin = [ define, undefine, defined, rawdef
          ...
          ++ [diagram name Asymptote exe (const "") (const "") | (name, exe) <- asymptoteExe <$> asymptoteDiagrams]
          ++ [diagram name R exe (("png('"++).(++"');")) (const "") | (name, exe) <- rExe <$> rDiagrams]
          ++
          [ lit, flushlit
          , source
          , codeblock
...
diagram name runtime exe header footer = Macro name []
    ...
    where
        impl env [path, title, code] = do
            ...
            let (src, dat, img, url, attrs) = parseImageAttributes env path'
            ...
            when (code'' /= oldCode || metaData /= oldMetaData) $ do
                ...
                void $ case runtime of
                    Graphviz ->
                        try readProcessUTF8 exe ["-Tpng", "-o", img, src]
                    PlantUML -> do
